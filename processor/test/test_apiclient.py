...
class TestApiClient(unittest.TestCase):
    ...
    def test_get_language_models_list(self):
        ...
        for p in models_list:
            assert "id" in p
            assert p["filename_prefix"] is not None, "{} has no filename_prefix".format(
                p["id"]
            )
            assert (
                p["filename_prefix"] not in prefixes
            ), "{} has a non-unique filename_prefix of {}".format(
                p["id"], p["filename_prefix"]
            )
