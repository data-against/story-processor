...
def fetch_project_stories_task(project_list: Dict, data_source: str) -> List[Dict]:
    ...
    for p in project_list:
        ...
        valid_stories = 0
        logger.info(
            "Project {}/{} - {} stories".format(p["id"], p["title"], len(feed.entries))
        )
        ...
        logger.info(
            "  project {} - {} valid stories (after {})".format(
                p["id"], valid_stories, history.last_publish_date
            )
        )
...
if __name__ == "__main__":
    ...
    def googlealerts_stories_flow(data_source: str):
        logger = get_run_logger()
        logger.info(
            "Starting {} story fetch job".format(processor.SOURCE_GOOGLE_ALERTS)
        )
        ...
        results_data = prefect_tasks.queue_stories_for_classification_task(
            projects_list, stories_with_text, data_source_name
        )
        # 5. send email/slack_msg with results of operations
        prefect_tasks.send_combined_slack_message_task(
            results_data, data_source_name, start_time
        )
        prefect_tasks.send_combined_email_task(
            results_data, data_source_name, start_time
        )
