...
def _domains_for_collection(cid: int) -> List[str]:
    ...
    while True:
        response = mc_directory_api.source_list(collection_id=cid, limit=limit, offset=offset)
...
def _project_story_worker(p: Dict) -> List[Dict]:
    ...
    try:
        total_hits = wm_provider.count(full_project_query, start_date, end_date, domains=p['domains'])
        time.sleep(0.2)  # don't hammer the API
        logger.info("Project {}/{} - {} total stories from {} domains (since {})".format(p['id'], p['title'],
                                                                                         total_hits, len(p['domains']), start_date))
        if total_hits > 0:  # don't bother querying if no results to page through
            ...
            for page in wm_provider.all_items(full_project_query, start_date, end_date, domains=p['domains'],
                                              page_size=PAGE_SIZE):
                ...
                for item in page:
                    if len(project_stories) > MAX_STORIES_PER_PROJECT:
                        break
                    info = dict(
                        id=item['id'],  # unique ID that can be used to fetch the pre-parsed text from the WM archive
                        ...
                        archived_url=item['archived_url'],  # the URL to the Wayback Machine provided HTML copy
                    )
                    project_stories.append(info)
                time.sleep(0.2)  # don't hammer the API
            logger.info("  project {} - {} stories (after {})".format(p['id'], len(project_stories),
...
def fetch_text(stories: List[Dict]) -> List[Dict]:
    ...
    def handle_parse(response_data: Dict):
        ...
        try:
            ...
            matching_input_stories = [s for s in stories if s['extracted_content_url'] == response_data['original_url']]
            ...
            logger.warning(f"Skipping story - failed to fetch due to {e} - from {response_data['original_url']}")
...
if __name__ == '__main__':
    ...
    def wayback_stories_flow(data_source: str):
        ...
        results_data = tasks.queue_stories_for_classification(projects_list, stories_with_text,
                                                              processor.SOURCE_WAYBACK_MACHINE, logger)
